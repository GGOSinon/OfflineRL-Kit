...
class COMBOIQLPolicy(CQLPolicy):
    """
    ...
    def learn(self, batch: Dict) -> Dict[str, float]:
        ...
        q_values = torch.min(q1a, q2a)
        v_values = self.value(obss)

        iql_weight = torch.where(q_values > v_values, self.iql_tau, (1 - self.iql_tau))
        value_loss = (iql_weight * torch.square(q_values - v_values)).mean()
